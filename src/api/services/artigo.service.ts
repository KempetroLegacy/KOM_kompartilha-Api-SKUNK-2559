...
export class ArtigoService {
  ...
  async findAll(): Promise<ArtigoResponseDTO[]> {
    const artigos = await this.artigoRepository.find({
      relations: ["author", "articleTags", "articleTags.tag"],
      order: { createdAt: "DESC" },
    });

    return artigos.map(ArtigoMapper.toResponseDTO);
  }
  ...
  async listDraftsByUser(userId: number) {
    const drafts = await this.articleDraftRepository.find({
      
      where: { author: { id: userId } },
      relations: ["author", "articleTags", "articleTags.tag"],
      order: { createdAt: "DESC" },
    });
    console.log("🚀 ~ ArtigoService ~ listDraftsByUser ~ drafts:", drafts)
    return drafts
  }

  async listDrafts(userId?: number): Promise<ArtigoResponseDTO[]> {
    ...
    return drafts.map(ArtigoMapper.toResponseDTO);
  }
  ...
  async search(query: string): Promise<ArtigoResponseDTO[]> {
